                   ...
                   FLOAT detectionThreshold, bool smoothScores)
{
  ...
  if (binarize)
  {
    ...
    cvThreshold(inImage, image, 0.1, 1, CV_THRESH_BINARY); //0.05
  }
  ...
  for (int i=0; i<image->width; i++)
    ...
    for (i=0; i<nzCount; i++)
      for (k=0; k<thetaBins; k++)
      {
        ...
        cout<<"value of rmin is "<<rMin<<endl;
        // cout<<"value of rmax is "<<rMin<<endl;
...
//     #endif
  ...
  cv::Mat my; 
   my = cv::Mat(ipm);
    cv::namedWindow("myipmimage");// defined in InversePers...Map.cc
    //cv::imshow("myipmimage",my);
    cv::imshow("myipmimage",my);
...
std::string final=st+str+".jpg";
addWeighted( my, 255, tmp, 0, 0.0, dst); 
//cv::imshow("myipmimage",dst);
 //cvSaveImage(final.c_str(), my) ;
  //cv::imshow("myipmimage",my);
   //cout<<my<<endl; 
 cv::imwrite(final,dst); 
 //cv::imwrite("orig"+final,image);
    cnt++;
    //cv::imshow("ipmimage",ipm);
  //  cout<<*image; 
 //return;
                 ...
                 LaneDetectorConf *lineConf, LineState *state)
{
  ...
  switch(lineConf->groupingType)
  {
    ...
    case GROUPING_TYPE_HV_LINES:
      //vector <Line> ipmStopLines;
      //vector <FLOAT> lineScores;
    cout<<endl<<"RISHUuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuUUUUUUUUUUUUUU"<<endl;
 ...
 void mcvDrawSpline1(CvMat *image, Spline spline, CvScalar color, int width)
{
  ...
  int x3=(cvGetReal2D(pixels, 7, 0));
  int y3=cvGetReal2D(pixels, 7, 1);
  int x4=(cvGetReal2D(pixels, 13, 0));
  int y4=cvGetReal2D(pixels, 13, 1);
  //cvCircle( image, cvPoint(x1,y1) , 8, CV_RGB(255,0, 0),-1 ); ///REPRESNET a POINTmove
  //cvCircle( image, cvPoint(x2,y2) , 8, CV_RGB(255, 0, 0),-1 );
  //cvCircle( image, cvPoint(x3,y3) , 8, CV_RGB(255, 0, 0),-1 ); ///REPRESNET a POINTmove
  //cvCircle( image, cvPoint(x4,y4) , 8, CV_RGB(255, 0, 0),-1 );
                        ...
                        CameraInfo &cameraInfo, CvSize imSize)
{
  ...
  cout<<"OKAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAY";
